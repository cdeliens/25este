...
//query_posts($query_string . '&category_name=articulo&posts_per_page=3');
$paged = (get_query_var('paged')) ? get_query_var('paged') : 1;
query_posts('paged=' . $paged . '&category_name=articulo&posts_per_page=3'.'&paged='.$paged); // show posts in category 76 with pagination enabled
...
if (array_key_exists('top',$list)) {
		print cooolzine_extended_boxarticle($list['top']['0']);	
}
else {
		print cooolzine_extended_boxarticle($list['top5']['0']);
		unset($list['top5']['0']);	
}
