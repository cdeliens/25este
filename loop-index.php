...
//verifica el link
$DirPath = getcwd();
$inner = basename(rtrim($DirPath, '/'));

if($inner=='cartelera'){
query_posts('category_name=Cartelera');
}else{
query_posts($query_string . '&cat=-182');
}

 //excluye categoria de 
//query_posts($query_string . '&cat=-182');
if (function_exists('dynamic_sidebar') && dynamic_sidebar('Sidebar-Ad Special')) {};
